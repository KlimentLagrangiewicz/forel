...
void fprintfResult(const char *fn, const int* const y, const int n, const double r) {
	FILE *fl  = fopen(fn, "a");
	if (fl == NULL) {
		printf("Error in opening %s result file...\n", fn);
		exit(1);
	}
	fprintf(fl, "Result of ForEl clustering with R = %.5lf ...\n", r);
	int i;
	for (i = 0; i < n; i++) {
...
void fprintfFullResult(const char *fn, const int* const y, const int n, const double a, const double r) {
	FILE *fl  = fopen(fn, "a");
	if (fl == NULL) {
		printf("Error in opening %s result file...\n", fn);
		exit(1);
